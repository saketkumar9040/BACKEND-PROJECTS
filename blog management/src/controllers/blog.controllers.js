...
exports.blogdata = async (req, res) => {
  try {
    ...
    if (req.body.isPublished === true) {
      //if document is set to published true it will create timestamp
...
exports.getBlog = async function (req, res) {
  try {
    ...
    if (Object.keys(query).length == 0) {
      //this block will work in case no filter is provided
    ...
    if (Object.keys(query).length > 0) {
      //this block will work in case filter is provided
      if (query.tags) {
...
exports.blogUpdate = async (req, res) => {
  try {
    ...
    if (Object.keys(data).length == 0) {
      //validation to check if body empty
    ...
    if (checkBlogId.isDeleted === true)
      return res.status(400).send({ msg: "Blog is deleted" });
    ...
    if (data.tags) {
      if (typeof data.tags === "object") {
        ...
        return res
          .status(400)
          .send({ status: false, msg: "Please send tags in array" });
      }
    }
    ...
    if (data.subcategory) {
      if (typeof data.subcategory === "object") {
        ...
        return res
          .status(400)
          .send({ status: false, msg: "Please send subcategory in array" });
      }
    }
